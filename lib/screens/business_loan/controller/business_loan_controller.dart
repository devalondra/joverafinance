...
class BusinessLoanState {
  const BusinessLoanState({
    ...
    required this.memorandumDocument,
    this.applicantType = "SME - Small, Medium Company",
  ...
  final DocumentModel memorandumDocument;
  ...
  BusinessLoanState copyWith({
    ...
    DocumentModel? memorandumDocument,
    ...
    return BusinessLoanState(
      ...
      memorandumDocument: memorandumDocument ?? this.memorandumDocument,
...
class BusinessLoanController extends StateNotifier<BusinessLoanState> {
  BusinessLoanController(this.ref)
    : super(
        BusinessLoanState(
          appLoadingController: AppLoadingController(),
          ...
          memorandumDocument: DocumentModel(),
        ),
      ) {
    calculateEMI();
  }
  ...
  DocumentModel get memorandumDocument => state.memorandumDocument;
  set memorandumDocument(DocumentModel value) =>
      state = state.copyWith(memorandumDocument: value);
  ...
  selectMemorandumDocument(context) {
    ...
    DocumentPicker().documentPickerWidget(
      ...
      () async {
        photoCopy = await pickfromGallery();
        memorandumDocument.filePath = getDocument(photoCopy);
        if (kDebugMode) print(tradeLicenseDocument.filePath);
      ...
      () async {
        final XFile? image = await pickfromCamera();
        memorandumDocument.filePath = getImage(image);
        initMemorandumDocument();
      },
    );
  }
  ...
  initMemorandumDocument() {
    if (memorandumDocument.fileName != "") {
      memorandumDocument.fileName =
          "memorandum.${memorandumDocument.filePath?.split('.').last}";
    }
    state = state.copyWith(memorandumDocument: memorandumDocument);
  }
...
final businessLoanControllerProvider =
    StateNotifierProvider<BusinessLoanController, BusinessLoanState>((ref) {
      return BusinessLoanController(ref);
    });
